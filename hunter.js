...
async function sRepeater(type) {
    ...
    let channels = ["1064359927499456542"]
    let ResultChannel = "1062659412164100136"
...
client.on(Events.InteractionCreate, async (interaction) => {
    try {
        ...
        let embed_quiz = await interaction.message.embeds[1];
        ...
        await interaction.message.edit({ embeds: [embed_prev, embed_quiz], files: [] })
    } catch (err) {
        console.log(err)
    }
})
...
async function generateRows(customId, monsterId, bgId, maxHP, name, answer) {
    ...
    const rands = [{
        ...
        customId: `attack:${customId}:${monsterId}:${bgId}:${maxHP}:${name}:${String(answer + 4)}`,
        emoji: "⚔️",
        btnStyle: ButtonStyle.Secondary,
        label: String(answer + 4),
    }, {
        customId: `attack:${customId}:${monsterId}:${bgId}:${maxHP}:${name}:${String(answer + 2)}`,
        emoji: "⚔️",
        btnStyle: ButtonStyle.Secondary,
        label: String(answer + 2),
    }, {
        customId: `attack:${customId}:${monsterId}:${bgId}:${maxHP}:${name}:${String(answer + 6)}`,
        emoji: "⚔️",
        btnStyle: ButtonStyle.Secondary,
        label: String(answer + 6),
    }]
    shuffle(rands)
...
client.login("MTA3MTAxNjMwNjI0NzIwODk5MA.GZCGPj.jKG7YW1a38ABbqHn0Uufmpl6HbkRecxSiAuJZk");